...
`r fpr_my_habitat_paragraph(loc = 'ds')` The stream channel is smaller and narrower than upstream. Abundant undercut banks for juvenile rearing. There were small gravels suitable for smaller fish. The habitat
...
`r fpr_my_habitat_paragraph(loc = 'us')` The water was tinged brown at the time of survey; likely from beaver activity upstream. No gravels were present suitable for spawning and the substrate was composed primarily of angular rock. Several rock steps of 50-60cm high were present in first 300m of survey that would prevent upstream juvenile salmonid migration. There was a crossing modelled upstream (ID 16601526) that was accessed to check for a structure and assess habitat. There was a large beaver dam approximately 1.6m in height at this location at the time of survey. Overall, the habitat surveyed upstream of the crossing
rated as `r fpr::fpr_my_priority_info(loc = 'us')` value as an important
migration corridor containing habitat suitable for spawning with moderate rearing potential.
...
`r fpr_my_pscis_info(col_pull = road_name)`. At crossing `r as.character(my_site)`, `r fpr::fpr_my_pscis_info()` is a
`r fpr::fpr_my_bcfishpass() %>% english::ordinal()` order stream with a
watershed area upstream of the crossing of approximately
`r fpr::fpr_my_wshd()`km^2^. The elevation of the watershed ranges from
a maximum of `r fpr::fpr_my_wshd(col = 'elev_max')`m to
`r fpr::fpr_my_wshd(col = 'elev_min')`m near the crossing (Table
\@ref(tab:tab-wshd-125261)).  At the time of reporting, there was no fisheries information available within provincial databases for the area upstream of crossing `r as.character(my_site)`. A total of `r fpr::fpr_my_bcfishpass(col_pull = total_lakereservoir_ha)`ha of lake and `r fpr::fpr_my_bcfishpass(col_pull = total_wetland_ha)`ha of wetland are modelled upstream. This includes the Fern Lakes, a collection of three lakes that have a combined area of approximately 138ha. There is a modelled crossing located 700m upstream of crossing `r as.character(my_site)`. However, upon visiting this location, there was no ford or crossing found. This site appears to be the end of an ATV trail. Located a further 3.3km upstream of this site is the outlet of the first Fern lake. There is only one other crossing modelled before the lake (ID 16601842). No assessments have been done at this location, so the crossing type and barrier status are unknown at this time.  
...
`r fpr::fpr_my_pscis_info(col_pull = barrier_result) %>% stringr::str_to_lower()`
to upstream fish passage according to the provincial protocol
[@moe2011] (Table \@ref(tab:tab-culvert-125261)). The culvert has baffles in it to assist upstream fish passage. However, rip rap at the outlet is creating a cascade approximately 0.5m in height that could block the migration of younger resident fish, depending on flow velocities. Water
temperature was
`r fpr::fpr_my_habitat_info(col_pull = 'temperature_c')`$^\circ$C, pH
was `r fpr::fpr_my_habitat_info(col_pull = 'p_h')` and conductivity was
`r fpr::fpr_my_habitat_info(col_pull = 'conductivity_m_s_cm')`uS/cm.
...
`r fpr_my_habitat_paragraph(loc = 'ds')` Extensive gravels were found in the first 150 m surveyed downstream. The habitat then transitions to wetland type with depths of up to 1m on the floodplain. The habitat
was rated as `r fpr::fpr_my_priority_info(loc = 'ds')` value for
salmonid rearing and spawning.
...
`r fpr_my_habitat_paragraph(loc = 'us')` This is a large stream with abundant gravel suitable for spawning and deep pools for overwintering. Numerous fish were observed, the habitat was stable with large woody debris found throughout, and abundant undercut banks. At the end of the survey, a modelled crossing was listed in `bcfishpass` (ID 16601842). Upon investigation, it was concluded that there was no crossing present `r if(gitbook_on){knitr::asis_output("(Figures \\@ref(fig:photo-125261-07) - \\@ref(fig:photo-125261-08)).")}else(knitr::asis_output("(Figure \\@ref(fig:photo-125261-u03))."))` Overall, the habitat surveyed upstream of crossing `r as.character(my_site)` was 
rated as `r fpr::fpr_my_priority_info(loc = 'us')` value as an important
migration corridor containing habitat suitable for spawning with moderate rearing potential.
...
my_caption1 = paste0('Downstream view at end of survey, at location of modelled crossing ', my_site2, '.')
```
...
my_caption2 = paste0('Upstream view at end of survey, at location of modelled crossing ', my_site2, '.')
